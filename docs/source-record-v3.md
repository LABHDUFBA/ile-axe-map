...
Validações de `date` e `uri` exigem `jsonschema.Draft202012Validator` com `jsonschema.FormatChecker()`. URLs aceitas são somente HTTP(S) absolutas com hostname não vazio. Domínios, IPv4, `localhost`, userinfo não vazio, caminho, query, fragmento e IPv6 válido entre colchetes, inclusive formas com IPv4 embedded, são aceitos sem consultar DNS. Userinfo pode conter `@` codificado como `%40`, mas não um segundo `@` cru. A porta opcional deve ser um inteiro decimal canônico entre `0` e `65535`: `0` é aceito, zeros à esquerda e sinais são rejeitados.
